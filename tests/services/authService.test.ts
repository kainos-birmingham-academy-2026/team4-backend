...
describe("AuthService - login", () => {
    ...
    it("should return a token when login is successful", async () => {
        ...
        const token = await authService.login({ email: mockUser.email, password: mockUser.password });
    ...
    it("should throw AuthError with status 401 when user is not found", async () => {
        ...
        const result = authService.login({ email: mockUser.email, password: mockUser.password });
        await expect(result).rejects.toThrow(new AuthError(401, "Invalid email or password"));
    ...
    it("should throw AuthError with status 401 when credentials are invalid", async () => {
        ...
        const result = authService.login({ email: mockUser.email, password: "WrongPassword123!" });
        await expect(result).rejects.toThrow(new AuthError(401, "Invalid email or password"));
    });
});
...
describe("AuthService - register", () => {
    ...
    it("should return a token when registration is successful", async () => {
        ...
        const token = await authService.register({ email: mockUser.email, password: mockUser.password });
    ...
    it("should throw AuthError with status 400 when user already exists", async () => {
        ...
        const result = authService.register({ email: mockUser.email, password: mockUser.password });
        await expect(result).rejects.toThrow(new AuthError(400, "User already exists"));
    });
    ...
    it("should throw an error when JWT_SECRET is not configured", async () => {
        ...
        const result = authService.register({ email: mockUser.email, password: mockUser.password });
        await expect(result).rejects.toThrow(new Error("JWT_SECRET has not been configured"));
    });
