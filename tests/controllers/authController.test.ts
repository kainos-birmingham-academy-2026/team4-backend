...
describe("AuthController - login", async () => {
    ...
    it("should return status 401 when login fails due to invalid credentials", async () => {
        mockAuthService.login = vi.fn().mockRejectedValue(new AuthError(401, "Invalid email or password"));
    ...
    it("should return status 500 when an unexpected error occurs", async () => {
        mockAuthService.login = vi.fn().mockRejectedValue(new Error("Unexpected error"));
...
describe("AuthController - register", async () => {
    ...
    it("should return status 400 when registration fails due to user already existing", async () => {
        mockAuthService.register = vi.fn().mockRejectedValue(new AuthError(400, "User already exists"));
    ...
    it("should return status 500 when an unexpected error occurs", async () => {
        mockAuthService.register = vi.fn().mockRejectedValue(new Error("Unexpected error"));
